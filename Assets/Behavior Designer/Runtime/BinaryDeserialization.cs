...
public static class BinaryDeserialization
{
    ...
    public static void Load(TaskSerializationData taskData, BehaviorSource behaviorSource, bool loadTasks)
    {
        ...
        taskIDs = null;
        if (!string.IsNullOrEmpty(taskData.Version)) {
    ...
    private static object LoadField(FieldSerializationData fieldSerializationData, Dictionary<int, int> fieldIndexMap, Type fieldType, string fieldName, int hashPrefix, IVariableSource variableSource, object obj = null, FieldInfo fieldInfo = null)
    {
        ...
        containsFieldHash = fieldIndexMap.TryGetValue(fieldHash, out fieldIndex);

        if (!containsFieldHash && fieldInfo != null) {
            UnityEngine.Serialization.FormerlySerializedAsAttribute formallySerializedAs;
            if ((formallySerializedAs = fieldInfo.GetCustomAttribute(typeof(UnityEngine.Serialization.FormerlySerializedAsAttribute), true) as UnityEngine.Serialization.FormerlySerializedAsAttribute) != null) {
                fieldHash = hashPrefix + StringHash(fieldType.Name.ToString(), strHashSerialization) + StringHash(formallySerializedAs.oldName, strHashSerialization);
                containsFieldHash = fieldIndexMap.TryGetValue(fieldHash, out fieldIndex);
            }
        }

        if (!containsFieldHash) {
#if NETFX_CORE && !UNITY_EDITOR
