...
namespace BehaviorDesigner.Runtime
{
    public class JSONDeserialization : UnityEngine.Object
    {
        ...
        private static void DeserializeObject(Task task, object obj, Dictionary<string, object> dict, IVariableSource variableSource, List<UnityEngine.Object> unityObjects)
        {
            ...
            for (int i = 0; i < fields.Length; ++i) {
                ...
                if (!dict.TryGetValue(fieldKey, out dictObject)) {
                    ...
                    if ((formallySerializedAs = fields[i].GetCustomAttribute(typeof(UnityEngine.Serialization.FormerlySerializedAsAttribute), true) as UnityEngine.Serialization.FormerlySerializedAsAttribute) != null) {
                        dict.TryGetValue(fields[i].FieldType.Name + formallySerializedAs.oldName, out dictObject);
                    }
                }
        ...
        private static Quaternion StringToQuaternion(string quaternionString)
        {
            ...
            return new Quaternion(float.Parse(stringSplit[0], CultureInfo.InvariantCulture), float.Parse(stringSplit[1], CultureInfo.InvariantCulture), float.Parse(stringSplit[2], CultureInfo.InvariantCulture), float.Parse(stringSplit[3], CultureInfo.InvariantCulture));
        }
